...
import { EntityArrayResponseType, SettingService } from '../service/setting.service';
...
export class SettingComponent implements OnInit {
  ...
  trackId = (_index: number, item: ISetting): string => this.settingService.getSettingIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.settings = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ISetting[] | null): ISetting[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.settingService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
